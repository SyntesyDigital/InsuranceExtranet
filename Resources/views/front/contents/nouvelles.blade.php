...
@section('content')
    ...
    <article class="content">
                ...
                <div class="row">
                    <div class="col-md-9 col-sm-10 col-xs-12 centered">
                        ...
                        @if ($fields['rotatorio']['value'] && count($fields['rotatorio']['value']) > 0)
                            @include('extranet::front.partials.fields.carousel_single', [
                        ...
                        @if (isset($fields['video']['value']['url'][App::getLocale()]))
                            @include('extranet::front.partials.fields.video', [
                            "field" => $fields['video'],
                            ])
                            @php $any_media = true; @endphp
                        @endif
                        ...
                        @if (!$any_media && $fields['imatge']['value'])
                            <img src="{{ isset($fields['imatge']['value']['urls']['large']) ? asset($fields['imatge']['value']['urls']['large']) : null }}"
        ...
        <div class="white">
                    ...
                    <div class="col-md-9 col-sm-10 col-xs-12 centered">
                        ...
                        <p class="details">
                            ...
                            @foreach ($categories as $cat)
                                ...
                                @endif
                                <a
                                    href="{{ route('blog.category.index', $cat->getFieldValue('slug')) }}">{{ $cat->getFieldValue('name') }}</a>
                            @endforeach
                        ...
                        @include('extranet::front.partials.fields.file',["field" =>$fields['pdf']])

                        <br />
                        <br />

                        <img src="{{ asset('modules/architect/images/comments.jpg') }}" />


                        <ul class="tags_blog">
                            @php $tags = $content->tags->all(); @endphp
                            @foreach ($tags as $tag)
                                <li href="{{ $tag->getFieldValue('slug') }}"><a
                                        href="{{ route('blog.tag.index', $tag->getFieldValue('slug')) }}">{{ $tag->getFieldValue('name') }}</a>
                                </li>
                            @endforeach
                        </ul>
            ...
            <div id="related-news" content="{{ $content->id }}"
                tags="{{ isset($content->tags) ? $content->tags->pluck('id') : null }}"
                category="{{ null !== $content->categories->first() ? $content->categories()->first()->id : null }}"></div>
