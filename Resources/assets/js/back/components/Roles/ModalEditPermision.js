...
class ModalEditPermision extends Component {
    ...
    getFirstGroup() {
        return (this.props.modal.groups !== undefined && this.props.modal.groups.length > 0) 
            ? this.props.modal.groups[0].id
            : '';
    }
    ...
    render() {
        ...
        for (var key in this.props.modal.groups) {
            var item = this.props.modal.groups[key] !== undefined 
                ? this.props.modal.groups[key] 
                : null;
